...
SUBROUTINE rhs_speed_eta(u,v,hleft,hdown,eta,f,rhs_u,rhs_v,rhs_eta)

  ! Sous-routine qui sort tous les RHS du modèle de Sadourny.
  ! IN : u,v,h,eta,f
  ! OUT : rhs_u,rhs_v,rhs_eta
  
  ! COMMENTAIRES :  
  ! - rhs_eta(:,:,3) est les plus récent, c'est donc sur lui qu'on va faire 
  ! nos modifications. rhs_eta(:,:,2) et rhs_eta(:,:,1) restent inchangées.
